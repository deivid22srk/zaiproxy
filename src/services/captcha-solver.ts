...
export class CaptchaSolver {
  ...
  async solve(account: ZaiAccount): Promise<string> {
    try {
      ...
      logger.error("AUTH", "Captcha solving requested, but Playwright is not installed.");
      throw new Error("FRONTEND_CAPTCHA_REQUIRED: Playwright is disabled or not installed, cannot solve captcha automatically.");
    }
  ...
  private async preparePage(session: CaptchaSession, account: ZaiAccount): Promise<any> {
    ...
    await page.evaluate(
      ({ localStorageValues, token }: { localStorageValues: Record<string, string>; token: string }) => {
  ...
  private async solveFresh(account: ZaiAccount): Promise<string> {
    ...
    try {
      ...
      const captcha = await page.evaluate(
        ({ language, scriptUrl, timeoutMs }: { language: string; scriptUrl: string; timeoutMs: number }) =>
