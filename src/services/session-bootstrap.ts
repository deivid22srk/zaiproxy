...
export class SessionBootstrap {
  ...
  async run(options: SessionBootstrapOptions = {}): Promise<void> {
    if (options.cookies) {
      await this.runCookieImport(options.cookies, options);
    } else {
      await this.runBrowserLogin(options);
    }
  }

  private async runCookieImport(source: string, options: SessionBootstrapOptions): Promise<void> {
    let rawContent = "";
  ...
  private async runBrowserLogin(options: SessionBootstrapOptions): Promise<void> {
    let playwright;
    try {
      playwright = await import("playwright");
    } catch {
      logger.error("AUTH", "Playwright is not installed. To login via Chrome in X11, install it using: npm install playwright");
      logger.info("AUTH", "Alternatively, import session via cookies: npm run bootstrap -- --cookies <file_or_url>");
      throw new Error("Playwright not installed and no cookies provided.");
    }

    if (!process.env.DISPLAY) {
      logger.error("AUTH", "DISPLAY environment variable is not set. An X11 server (VNC / Termux-X11) is required for headed Chrome login.");
      logger.info("AUTH", "Please run in X11 environment or import cookies using: npm run bootstrap -- --cookies <file_or_url>");
      throw new Error("DISPLAY environment variable is missing.");
    }

    const profileName = this.selectProfileName(options);
    const profilePath = ensureDir(join(config.runtimeDir, "profiles", profileName));
    logger.info("AUTH", `Opening Chromium profile at ${profilePath} on DISPLAY ${process.env.DISPLAY}`);
...
async function waitForRealToken(page: any, timeoutMs: number, allowGuest: boolean): Promise<string> {
...
async function safeReadToken(page: any): Promise<string | null> {
  try {
...
async function readLocalStorage(page: any): Promise<Record<string, string>> {
  try {
